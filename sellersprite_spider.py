...
class SellerSpriteSpider:
    ...
    async def _navigate_to_product_research(self):
        ...
        # 添加销量区间过滤
        if config.MIN_SALES is not None:
            url += f"&minSales={config.MIN_SALES}"
        if getattr(config, 'MAX_SALES', None) is not None:
            url += f"&maxSales={config.MAX_SALES}"
            
        logger.info(f"导航到产品调研页面...")
    ...
    async def _crawl_all_data(self):
        ...
        while self.current_page < total_pages:
            ...
            # 定期保存 (每50页保存一次 checkpoint)
            if self.current_page % 50 == 0:
                self._save_data(suffix=f"_page_{self.current_page}")
    ...
    def _save_data(self, suffix=""):
        ...
        try:
            ...
            logger.info(f"  列名: {list(df.columns[:20])}{'...' if len(df.columns) > 20 else ''}")
        except Exception as e:
